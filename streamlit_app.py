...
if dresses_csv and jeans_csv:
    # Detect delimiter for dresses
    sample = dresses_csv.read(1024).decode("utf-8")
    dresses_csv.seek(0)
    dialect = csv.Sniffer().sniff(sample)
    dresses_df = pd.read_csv(dresses_csv, delimiter=dialect.delimiter, encoding="utf-8")

    # Detect delimiter for jeans
    sample = jeans_csv.read(1024).decode("utf-8")
    jeans_csv.seek(0)
    dialect = csv.Sniffer().sniff(sample)
    jeans_df = pd.read_csv(jeans_csv, delimiter=dialect.delimiter, encoding="utf-8")
